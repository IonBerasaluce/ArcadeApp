#include <cmath>
#include "Shapes/Line2D.h"
#include "Math/Vec2D.h"
#include "Utils/Utils.h"

Line2D::Line2D():Line2D(Vec2D::Zero, Vec2D::Zero)
{}

Line2D::Line2D(float x0, float y0, float x1, float y1):Line2D(Vec2D(x0, y0), Vec2D(x1, y1))
{}

Line2D::Line2D(const Vec2D& p0, const Vec2D& p1) : mP0(p0), mP1(p1)
{}

float Line2D::MinDistanceFrom(const Vec2D& p, bool limitToSegment /*= false*/) const
{
	return p.Distance(ClosestPoint(p, limitToSegment));
}

Vec2D Line2D::ClosestPoint(const Vec2D& p, bool limitToSegment /*= false*/) const
{
	Vec2D p0ToP = p - mP0;

	Vec2D p0ToP1 = p - mP1;

	float l2 = p0ToP1.Mag2();

	float dot = p0ToP.Dot(p0ToP1);

	float t = dot / l2;

	if (limitToSegment)
	{
		t = std::fmax(0, std::fmin(1.0f, t));
	}

	return mP0 + p0ToP1 * t;
}

Vec2D Line2D::Midpoint() const
{
	return (mP0 + mP1) / 2.0f;
}

float Line2D::Slope() const
{
	float dx = mP1.GetX() - mP0.GetX();
	float dy = mP1.GetY() - mP0.GetY();
	
	if (dx < EPSILON)
	{
		return 0;
	}

	return  dy / dx;
}

float Line2D::Length() const
{
	return mP1.Distance(mP0);
}

void Line2D::rotate(float angle)
{
	Vec2D midPoint = Midpoint();
	mP0.Rotate(angle, midPoint);
	mP1.Rotate(angle, midPoint);
}

Line2D Line2D::rotateWithResult(float angle) const
{
	Vec2D midPoint = Midpoint();
	Vec2D new_mP0 = mP0.RotationResults(angle, midPoint);
	Vec2D new_mP1 = mP1.RotationResults(angle , midPoint);

	Line2D newLine = { new_mP0, new_mP1 };

	return newLine;
}

bool Line2D::operator==(const Line2D& line) const
{
	return line.GetP0() == mP0 && line.GetP1() == mP1;
}
